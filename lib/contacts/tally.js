const FIELD_ID_ALIASES = {
  ...
  // IDs without labels, so keep these server-side aliases as a fallback.
...
function fieldName(field) {
  const rawName = cleanString(field?.key || field?.label || field?.title || field?.name || field?.question || field?.id);
  return FIELD_ID_ALIASES[normalizeAliasKey(rawName)] || normalizeKey(rawName);
}
